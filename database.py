...
def init_db():
    ...
    tables = [
        ...
        """CREATE TABLE IF NOT EXISTS athletes (
            ...
            sport TEXT, position TEXT, year TEXT, notes TEXT
        )""",
        """CREATE TABLE IF NOT EXISTS programs (
            id SERIAL PRIMARY KEY,
            coach_id INTEGER REFERENCES users(id),
            name TEXT NOT NULL, description TEXT, goal TEXT, weeks INTEGER,
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
        )""",
        """CREATE TABLE IF NOT EXISTS phases (
            id SERIAL PRIMARY KEY,
            program_id INTEGER REFERENCES programs(id) ON DELETE CASCADE,
            name TEXT NOT NULL, week_start INTEGER, week_end INTEGER,
            sets TEXT, reps TEXT, rpe TEXT, notes TEXT, sort_order INTEGER DEFAULT 0
        )""",
        """CREATE TABLE IF NOT EXISTS workout_days (
            id SERIAL PRIMARY KEY,
            phase_id INTEGER REFERENCES phases(id) ON DELETE CASCADE,
            day_number INTEGER, title TEXT, focus TEXT, sort_order INTEGER DEFAULT 0
        )""",
        """CREATE TABLE IF NOT EXISTS exercises (
            id SERIAL PRIMARY KEY,
            day_id INTEGER REFERENCES workout_days(id) ON DELETE CASCADE,
            name TEXT NOT NULL, sets TEXT, reps TEXT, notes TEXT, sort_order INTEGER DEFAULT 0
        )""",
        """CREATE TABLE IF NOT EXISTS exercise_library (
            id SERIAL PRIMARY KEY,
            name TEXT NOT NULL, category TEXT, muscle_group TEXT,
            description TEXT, coaching_cues TEXT
        )""",
        ...
        """CREATE TABLE IF NOT EXISTS workout_logs (
            ...
            sets_done INTEGER, reps_done TEXT, weight TEXT, rpe_actual TEXT, notes TEXT
        )""",
    ]
